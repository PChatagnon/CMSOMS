...
class RuntimesController extends Component {
        ...
        switch (state.filterBy) {
            case 'cms_fillRange':
                Object.keys(state).forEach(key => {
                    if (!(['cms_fill_to', 'cms_fill_from', 'cms_fill_stableOnly'].includes(key))) delete state[key];
                });
                break;
            case 'cms_dateRange':
                Object.keys(state).forEach(key => {
                    if (!(['cms_date_to', 'cms_date_from', 'cms_fill_stableOnly'].includes(key))) delete state[key];
                });
                break;
            default:
                break;
...
export default withStyles(styles)(RuntimesController);
