...
class DowntimesLostLumiByGroupChart extends Component {
    ...
    constructor(props) {
        ...
        this.id = generateId('downtimesLostLumiByGroupChart');
    }
        ...
        this.props.fetchData(this.props.configuration.endpoint, this.params)
            .then(resp => {
                ...
                resp.data.data.forEach(row => {
                    const { group, lost_lumi, duration } = row.attributes;

                    if (!groups[group]) groups[group] = { time: 0, lumi: 0 };

                    groups[group].lumi += lost_lumi ? lost_lumi : 0;
                    groups[group].time += duration ? duration : 0;
                });

                Object.keys(groups).forEach(g => {
                    this.lumiData.push({ name: g, y: groups[g].lumi });
                    this.timeData.push({ name: g, y: groups[g].time });
                });
...
export default sizeMe({ monitorWidth: true })(DowntimesLostLumiByGroupChart);
