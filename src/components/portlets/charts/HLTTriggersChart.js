...
class HLTTriggersChart extends Component {
    ...
    componentDidUpdate(prevProps) {
        this.props.shouldUpdate(prevProps.query, this.props.query, () => {
            ...
            this.cancelRequests("HLTTriggersChart: new Query");
    ...
    componentWillUnmount() {
        this.cancelSource.cancel("HLTTriggersChart: component is unmounting");
        this._isMounted = false;
        this.chart.destroy();
    ...
    shouldUpdate(prevProps) {
        ...
        switch (this.mdData.selector) {
            case 'hlt_trigger_path':
                id = this.mdData.row.attributes.path_name;
                this.loadData(id, this.addHLTRatesSeries);
                return;
            default:
                return;
        }
    ...
    loadData = (id, fetchSeriesData) => {
        ...
        if (this.mdData.selected && !series) {
            this.clearTimer();
            fetchSeriesData(id, this.mdData.row.attributes.path_name) //name
                .then(() => {
        ...
        if (!this.mdData.selected) {
            ...
            else this.cancelRequests('HLTTriggersChart: Pending request was canceled by deselecting MD row.');
        ...
        Promise.all(
            this.chart.series.map(series => {
                ...
                switch (seriesData.type) {
                    case 'hltTriggerRates':
                        updateFunction = this.updateHLTRatesSeries;
                        break;
                    default:
                        updateFunction = null;
                        break;
    ...
    addHLTRatesSeries = (id, label) => {
        ...
        return this.props.fetchData('hltpathratesperlumisection', params)
            .then(resp => {
                ...
                this.series.push({ id: id, 'lumi': lumiData, 'utc': utcData, type: 'hltTriggerRates', lastLumiIndex: lastLumiIndex });
...
export default sizeMe({ monitorWidth: true })(
    connect(mapStateToProps)(HLTTriggersChart)
);
