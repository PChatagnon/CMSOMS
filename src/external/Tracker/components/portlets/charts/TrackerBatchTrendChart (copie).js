...
class TrackerBatchTrendChart extends Component {
    ...
    createSQL = (element, sqli, sqlf, i) => {
        ...
        sqlf = sqlf.replace(partBarcode, "'" + element.barcode + "'")
        sqlf = sqlf.replace(runTypeNumber, "'" + element.runNumber + "'")
        sqlf = sqlf.replace("tracker_hmSetType", "'" + element.Set + "'")
        sqlf = sqlf.replace("tracker_hmConfigType", "'" + element.Config + "'")
